...
// ● A - Função para retornar apenas o funcionário com o id informado:
const employeeById = id => employees.find(({ id: uid }) => id === uid)

// ● B - Função para filtrar apenas os funcionários ativos na empresa:
const employeesActive = () => employees.filter(({ active }) => active)

// ● C - Função para filtrar apenas funcionários clt
const employeesCLT = () => employees.filter(({ type }) => type === 'clt')

// ● D - Função para criar uma nova estrutura onde cada funcionário receberá uma nova informação de base salarial
const createNewStructure = (atribute, defaultValue) =>
    employees.map(value => {
...
const employeesGroups = () =>
    employees.reduce(
        (groups, employer) => {
            ...
            console.log(clt);
            if (employer.type == 'clt') clt.push(employer.name)
            else if (employer.type == 'pj') pj.push(employer.name)
...
const activeEmployeesCrescent = () => {
    return employeesActive().sort((a, b) => a.age - b.age)
}

const resultados = [{
        text: "Funcionário com o id informado:",
        array: employeeById(2),
    },
    {
        text: "Funcionários ativos na empresa:",
        array: employeesActive(),
    },
    {
        text: "Funcionários CLT",
        array: employeesCLT(),
    },
    {
        text: "Nova estrutura onde cada funcionário receberá uma nova informação de base salarial",
        array: createNewStructure('salary', 1100),
    },
    {
        text: "Funcionários PJ e CLT",
        array: employeesGroups(),
    },
    {
        text: "Funcionários ativos ordenados por ordem de idade",
        array: activeEmployeesCrescent(),
    },
]
